...
We minimized any changes to the data processing and analysis procedures used in the reproduction attempt because we wish to assess whether we can computationally reproduce the results of the original authors.
...
Executing the attempt will also allow us to identify questions about the approach of the original authors.
...
2. a binary indicator that identified if the PA closest to an unprotected site as larger than 500 sq km
...
Using propensity score matching to control for the confounds of location, site accessibility, and forest structure, Brodie et al. fit a linear mixed-effects models to estimate the PA related effects. 
...
In total, the authors fit six separate regression models - two taxons (birds, mammals) for three biodiversity measures (SR, FR, PD) - testing the null hypothesis that:
...
Brodie et al. find evidence that the legal designation of PAs provides statistically significant benefits to Southeast Asian bird biodiversity.
...
Brodie et al. find evidence that large PAs are associated with higher biodiversities for mammals and birds in surrounding unprotected areas. 
However, size effects were only observed for two of the three response variables for birds.
The authors found that distance to the nearest PA was significantly associated with only the species richness of birds.
...
Weights produced by propensity score matching were then used in mixed-effects linear regression models that estimated the treatment effect - PA status, nearest PA size, or nearest PA distance - while adjusting for forest canopy height, site accessibility, and HDI (Table 1).


## Observations Preceeding the Reproduction Attempt  

Before beginning our reproduction attempt, we observed the analysis file and code published by Brodie et al..
We identified the following issues in the published analysis file, associated metadata, and script.

1) Only the analysis file was made available. Source files that permit examination and extension of data processing procedures were not published.
2) The procedure for computing the three main response variables was not presented in the script.
3) The procedure for constructing the three main treatment variables was unclear. Specifically, information about how PA boundaries were processed for analysis was not provided.
4) The procedures for computing predictor variables designed to control for key confounds of protection - forest structure, the circuit-based accessibility - were not provided in the scripts and not presented in detail in the methodological supplement.
5) The procedure for identifying and eliminating outliers was not included in the script. A hand coded list of outliers was provided and removed in the code.
6) The HDI measure was missing from the original analysis file. The script and analysis file published by the authors with their correction in April 2024 addressed this issue. 
7) Metadata provided minimal information about the data and did not follow a widely adopted standard.
...
We were able to statistically reproduce the results of Brodie et al. using an unaltered version of the their corrected data and code files.
...
The computational setup for the analysis was:
...
Our results (Table 3) correspond to the top third of Table 1 of the corrected supplemental materials presented by Brodie et al.. 
...
matchb_birds_size_pd <- MatchIt::matchit(BigPA ~ utm_north.z + utm_east.z + gedi.m + access_log10.z + HDI.z + dist_to_PA.z, 
                                         data = d_birds3b, 
                                         method = "full", 
                                         distance = "glm",
                                         link = "probit", 
...
match2_birds_dist_pd <- MatchIt::matchit(CloseToPA ~ utm_north.z + utm_east.z + gedi.m + access_log10.z + HDI.z + PA_size_km2.z, 
                                         data = d_birds3b, 
                                         method = "full", 
                                         distance = "glm",
                                         link = "probit", 
...
mm02d_birds_dist_pd <- nlme::lme(y ~ gedi.m + access_log10.z + HDI.z + PA_size_km2.z + CloseToPA,
                                 random = list(~1 | country),
                                 data = dmatch_birds_dist_pd, 
                                 weights = ~I(1/weights), 
...
Our statistical reproduction produced results that were consistent with the main findings of Brodie et al. (2023). 
Our numerical results similarly supported rejection of OR-H0-1 (i.e., similar coefficient values and significance), although we were unable to reproduce exact results as Brodie et al. (2023).
Our numerical results similarly supported rejection of OR-H0-2a and a failure to reject OR-H02b.

While our numerical results aligned with those of Brodie et al., the limited information about how variables were constructed restricted our ability to directly assess the outcomes and interpretations of the original paper. 
The scripts can be used to recreate the numerical results, but did not include data preprocessing steps such as the procedure for cleaning protected area boundaries, or computing secondary variables from raw data (e.g., GEDI metrics and circuit-based accessibility metrics).
Provided datafiles also did not include raw observation data needed to recreate variables, or an extensive metadata file documenting those data sources.
Omission of the processing steps used to create the analytical datafile meant that we could only directly assess the statistical analysis of the authors, as we could not retrace the variable creation process. 

Similarly, because procedural information and code were not provided for the authors' figures, we were only able to partially reproduce these elements. 
For example, we were unable to recreate the estimated mean differences in biodiversity levels reported in Figures 3, 4, and 5. 
This issue may be due to our use of the authors corrected data file and code. 
While the authors updated these components of the project, we were unable to find correspondingly updated figures of difference estimates. 
Those published in the paper appear to be estimated from the authors original models, and therefore do not reflect the new models used in the correction and in our reproduction attempts. 
We were unable to verify this issue using the original data because of the incomplete nature of that file. 

Finally, we were unable to identify how the authors processed their spatial PA data. 
We were unable to determine whether the authors merged and refined PAs that in the original data set are administratively distinct, but geographically adjacent. 
We were also unable to identify whether the authors removed marine overlaps from their terrestrial PAs. 
These issues may impact the original estimation of size-based PA spillover effects. 


## Bias and threats to validity

There remain minor concerns about statistical bias and construct validity.
The original models account for location and potential spatial autocorrelation in both the matching procedure and the clustering of errors in regression models.
However, the authors did not present any formal testing of remaining spatial autocorrelation in the residuals. 
The paper does not include an extensive discussion of measurement uncertainty issues that could lead to concerns about construct validity (e.g., biodiversity measures, forest structure measures created through kriging,)
Independent investigation of these issues cannot be easily undertaken given the lack of documentation of variable construction and processing.


# Acknowledgements

This report is based upon the template for Reproducible and Replicable Research in Human-Environment and Geographical Sciences, <https://doi.org/10.17605/OSF.IO/W29MQ>.
