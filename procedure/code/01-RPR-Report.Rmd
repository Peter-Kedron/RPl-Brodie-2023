...
```{r reproduction_efficacy, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}
# Make a common catalog
var_catalog <- data.frame(
    response_variable = c("asymptPD", "maxFRic", "SR.mean"),
    name = c("PD", "FR", "SR"))

# Reproduce models for bird
rep_models_bird <- lapply(1:nrow(var_catalog), function(i){
    model_pa_efficacy(dat_clean_bird, "brodie", "bird",
                      var_catalog[[i, "response_variable"]], 
                      outliers_eff_bird[[var_catalog[[i, "name"]]]])
}); names(rep_models_bird) <- sprintf("mod_bird_eff_%s", var_catalog$name)

# Reproduce models for mammal
rep_models_mammal <- lapply(1:nrow(var_catalog), function(i){
    model_pa_efficacy(dat_clean_bird, "brodie", "mammal",
                      var_catalog[[i, "response_variable"]], NULL)
}); names(rep_models_bird) <- sprintf("mod_mammal_eff_%s", var_catalog$name)
```
...
```{r reproduction_spillover, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}
# Reproduce models for bird
rep_models_bird <- lapply(1:nrow(var_catalog), function(i){
    mods <- lapply(c("BigPA", "CloseToPA"), function(bnr_var){
        model_pa_spillover(dat_clean_bird, "brodie", "bird", bnr_var, 
                       var_catalog[[i, "response_variable"]], 
...
rep_models_bird <- do.call(c, rep_models_bird)

# Reproduce models for mammal
rep_models_mammal <- lapply(1:nrow(var_catalog), function(i){
    mods <- lapply(c("BigPA", "CloseToPA"), function(bnr_var){
        model_pa_spillover(dat_clean_bird, "brodie", "mammal", bnr_var, 
                       var_catalog[[i, "response_variable"]], 
...
rep_models_mammal <- do.call(c, rep_models_mammal)
```
