...
# Wenxin add code loading data here

# Load and Clean Original Data -------------------------------------------------

# Copy outlier sets used by Brodie et al.
outliers_eff_bird <- list("PD" = c("L2422371", "L3776738", "L2521761", 
                                   "L6127181", "L3865754"),
                          "FR" = c("L921125", "L2422371", "L4331944", 
                                   "L13465594"),
                          "SR" = c("L4789498", "L921125", "L1122096", 
                                   "L7010824", "L3865754", "L3776738"))

outliers_spil_bird <- list("PD" = c("L1084299", "L4225511", "L3846512", 
                                    "L2129865", "L3267752"),
                           "FR" = c("L4225511", "L5969878", "L3267752", 
                                    "L4331944", "L13465594", "L1084299"),
                           "SR" = c("L4225511", "L5624588", "L3321319", 
                                    "L14087870"))

outliers_spil_mammal <- list("PD" = c("WM-OP009", "WM-HCV003", "C24A25", 
                                      "C1A09", "C1B12"),
                             "FR" = c("Bal013a", "Bal017a", "C1CT21"),
                             "SR" = c("Bal011", "C1CT50", "C24A25"))

# Remove outliers from bird and mammal models
...
```{r rpr_cor_eff, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}

```

```{r rpr_cor_eff_tab, echo = TRUE, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r rpr_cor_spill, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}

```

```{r rpr_cor_spill_tab, echo = TRUE, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r rpr_org_eff, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}

## Estimate PA efficacy (OR-Ho-1) using original data and code -----------------
...
rpr_eff_bird <- lapply(1:nrow(var_catalog), function(i){
    model_pa_efficacy(dat_clean_bird, "brodie", "bird",
                      var_catalog[[i, "response_variable"]], 
                      outliers_eff_bird[[var_catalog[[i, "name"]]]])
}); names(rpr_eff_bird) <- sprintf("mod_bird_eff_%s", var_catalog$name)

# Reproduce models for mammals
rpr_eff_mammal <- lapply(1:nrow(var_catalog), function(i){
    model_pa_efficacy(dat_clean_mammal, "brodie", "mammal",
                      var_catalog[[i, "response_variable"]], NULL)
}); names(rpr_eff_mammal) <- sprintf("mod_mammal_eff_%s", var_catalog$name)
```

```{r rpr_org_eff_tab, echo = TRUE, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r rep_org_spill, echo = TRUE, results = 'hide', message = FALSE, eval = FALSE}

## Estimate PA efficacy (OR-Ho-2a,b) using original data and code --------------

# Reproduce models for birds
rpr_spill_bird <- lapply(1:nrow(var_catalog), function(i){
...
rpr_spill_bird <- do.call(c, rpr_spill_bird)

# Reproduce models for mammals
rpr_spill_mammal <- lapply(1:nrow(var_catalog), function(i){
    mods <- lapply(c("BigPA", "CloseToPA"), function(bnr_var){
        model_pa_spillover(dat_clean_mammal, "brodie", "mammal", bnr_var, 
                       var_catalog[[i, "response_variable"]], 
...
rpr_spill_mammal <- do.call(c, rpr_spill_mammal)
```

```{r rpr_org_spill_tab, echo = TRUE, message = FALSE, warning = FALSE, eval = FALSE}
