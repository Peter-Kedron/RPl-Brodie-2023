...
options(tinytex.verbose = TRUE) # not sure if this line is needed
...
Debates exist on whether protected areas are effective because of their remote location and good habitat conditions or conservation status itself. Motivated by such, Brodie et al. (2023) estimated protected areas efficacy for conserving tropical mammal and bird diversity after de-confounding the effects of site accessibility and habitat condition (OR-H1). To test the hypothesis, Brodie et al. (2023) used various aspects of biodiversity as outcome variables (i.e., dependent variable). The treatment variable is a binary variable on whether the observation point is within or outside a reserve. If the treatment variable is significant in a causal model, it supports OR-H1 and adds evidence for the effectiveness of protecting lands.

To test OR-H1, the original study removed confounding effects and built separate causal models for birds and mammals, and different biodiversity outcome metrics. Two major confounders identified were site accessibility and habitat quality. Site accessibility was proxied using Human Development Index (HDI), circuit theory-based measures of proximity to human development^["circuit theoretical models parameterized with human travel speeds across different terrains and the locations of populations centers and transportation networks" (Brodie et al. 2023)], and an interaction term of the two variables. Habitat quality was proxied using three-dimensional habitat structure metrics derived from the Global Ecosystem Dynamics Investigation (GEDI) mission. Confounding effects were removed using statistical matching based on propensity scores. Biodiversity metrics used as outcome variables include species richness (SR), functional richness (FR), and phylogenetic diversity (PD). PA effects were estimated using separate mixed effects linear regression for different biodiversity metrics and for birds vs mammals. The original study archetype is quasi-experimental as it uses a matching strategy to perform causal analysis.
...
## Testing the moderating effect of connectivity

Connectivity of protected areas is essential for biodiversity conservation which supports species movement and facilitates energy transfer that is essential for species survival. Well-connected protected areas theoretically have higher effectiveness in conserving biodiversity, and moderate the effect of conservation status itself. It could potentially be an important contributing factor to high tropical bird and mammal biodiversity, but was not included in the original study. Therefore, we introduce connectivity measures in our replication work and develop the hypothesis (RPL-H1) as follows.

> RPL-H1: Connectivity mdoerates the efficacy of protection on biodiversity.

We ask the research question of whether connectivity moderates the efficacy of protection on tropical biodiversity and build upon the existing structural causal models by Brodie et al. (2023) to test the hypothesis. 
...
The reproduction of the original study was conducted in MacBook Pros using R programing language (Version 4.2.2). Brodie et al. (2023) cleaned up the variables and performed propensity score matching and causal analysis in R. They did not provide scripts on how they derived the biodiversity metrics, circuit theory-based metrics, HDI, and the GEDI metrics. We built upon and annotated R scripts shared by them, and added missing information on HDI to the scripts.
// Is this accurate?
...
We performed a series of data pre-processing steps before building the causal models, which include 1) cleaning protected area boundaries, 2) re-construcitng the causla diagram, and 3) cleeaning up data files produced by Brodie et al. (2023).

### Clean protected areas (takes too long to run so I skipped many lines --> should discuss)
...
# Wenxin: Was this (or the entire PA cleaning) done only for connectivity analysis? Should I still include them in the reproduction report?
...
dat_FR_efficacy_bird <- subset(dat_clean_bird, med_dist == 100)
...
dat_SR_efficacy_bird <- subset(dat_clean_bird, med_dist == 100) # Wenxin: why is the med_dist different here? should we use the same distance for all measures?  It does not really matter for the reproduction though.
...
msummary(list('Bird SR'=mod_SR_efficacy_bird, 'Bird FR'=mod_FR_efficacy_bird, 'Bird PD'=mod_PD_efficacy_bird, 'Mammal SR'=mod_SR_efficacy_mam, 'Mammal FR'=mod_FR_efficacy_mam, 'Mammal PD'=mod_PD_efficacy_mam),stars = TRUE)
